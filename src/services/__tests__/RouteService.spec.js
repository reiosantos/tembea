...
describe('RouteService', () => {
  ...
  const destination = { address: 'BBBBBB' };
  const route = {
    name: 'ZZZZZZ', destination, routeBatch: [{ batch: 'A' }],
  };
  const cabDetails = {
  ...
  describe('RouteService_createRouteBatch', () => {
    ...
    beforeEach(() => {
      jest.spyOn(AddressService, 'findAddress').mockResolvedValue(destination);
  ...
  describe('RouteService_createRoute', () => {
    ...
    it('should return created route details', async () => {
      const name = 'yaba';
      const res = await RouteService.createRoute(name, routeTwo);
